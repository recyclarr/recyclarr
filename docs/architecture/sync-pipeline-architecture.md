...
Preview (dry-run) is an adapter concern, not a sync engine concern. In preview mode, the pipeline
runs `Compute()` for every operation but skips `Persist()`. Compute results go into the run's
storage and are read back when the run completes.

`SyncJobRunner` calls `ISyncRunResults.GetInstanceResult()` to retrieve typed per-operation results
and records them on the job before the run scope is disposed. The server maps them to JSON DTOs for
`GET /sync/jobs/{id}/results` (ADR-014), and the CLI dispatches that JSON to per-operation renderer
classes that format the output using Spectre.Console tables and trees.
