...
) : IGitRepositoryService
{
    ...
    )
    {
        ...
        var tasks = allRepositoryDefinitions.Select(async definition =>
        {
            ...
            progress?.Report(
                new RepositoryProgress(repoType, gitRepo.Name, RepositoryProgressStatus.Starting)
            );

            try
            {
                progress?.Report(
                    new RepositoryProgress(repoType, gitRepo.Name, RepositoryProgressStatus.Cloning)
                );
