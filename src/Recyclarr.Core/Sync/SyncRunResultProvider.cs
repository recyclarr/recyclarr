...
internal class SyncRunResultProvider(ISyncRunStorage storage) : ISyncRunResults
{
    public SyncInstanceResult GetInstanceResult(string instanceName)
    {
        var naming = storage.Retrieve(instanceName, PipelineType.MediaNaming);

        return new SyncInstanceResult
        {
            CustomFormats = MapCustomFormats(
                storage.Retrieve(instanceName, PipelineType.CustomFormat)
            ),
            QualityProfiles = MapQualityProfiles(
                storage.Retrieve(instanceName, PipelineType.QualityProfile)
            ),
            QualitySizes = MapQualitySizes(
                storage.Retrieve(instanceName, PipelineType.QualitySize)
            ),
            ...
            MediaManagement = MapMediaManagement(
                storage.Retrieve(instanceName, PipelineType.MediaManagement)
            ),
        };
    }
