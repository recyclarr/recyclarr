...
internal partial class QualityProfileViewModel : WizardStepViewModel
{
    ...
    private bool HasSelection => SelectedFlagValue is not null and not 0;
    ...
    public override (string Title, string Message)? GetAdvanceConfirmation()
    {
        if (HasSelection)
        {
            return null;
        ...
        return (
            "Skip Quality Profiles?",
            "No quality profiles selected. Custom format groups will also be skipped.\n\nContinue without quality profiles?"
        );
    }

    public override void Activate()
    {
        // Dispose old subscription before populating so the null reset
        // in PopulateSelector doesn't fire a stale sync that wipes wizard state.
