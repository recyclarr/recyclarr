...
internal sealed class CfGroupStepView : WizardStepViewBase<CfGroupViewModel>
{
    ...
    private static FrameView CreateGroupFrame(string title, Pos x, Dim width)
    {
        // NoStop on checkboxes so arrow keys navigate items; Tab moves between panels
        var selector = new FlagSelector { Width = Dim.Fill(), TabBehavior = TabBehavior.NoStop };
