...
internal class QualitySizeConfigPhase(
    ILogger log,
    ResourceProviders.Domain.QualitySizeResourceQuery guide,
...
) : IPipelinePhase<QualitySizePipelineContext>
{
    ...
    )
    {
        ...
        IEnumerable<QualitySizeResource> qualitySizes = config.ServiceType switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(config.ServiceType)),
        };

        var guideSizeData = qualitySizes.FirstOrDefault(x =>
