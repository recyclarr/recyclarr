...
internal sealed class PlanBuilderQualityProfileTest : PlanBuilderTestBase
{
    ...
    ) => GuideData.AddQualityProfile(trashId, name, qualities);
    ...
    ) => GuideData.AddQualityProfileWithFormatItems(trashId, name, trashScoreSet, formatItems);

    private void SetupCustomFormatWithScores(
        string name,
        string trashId,
        params (string ScoreSet, int Score)[] scores
    ) => GuideData.AddCustomFormatWithScores(name, trashId, scores);
    ...
    public void Build_with_invalid_quality_profile_trash_id_reports_warning()
    {
        ...
        plan.Outcomes.Should()
            .ContainSingle()
            .Which.Should()
            .Be(new InvalidQualityProfileTrashIdOutcome("nonexistent-qp"));
    }
    ...
    public void Duplicate_profile_names_reports_error_and_skips_both()
    {
        ...
        plan.Outcomes.Should()
            .ContainSingle()
            .Which.Should()
            .Be(new DuplicateQualityProfileNameOutcome("Same Name"));
    }

    [Test]
    public void Custom_formats_sharing_service_id_retain_collision_context()
    {
        var registry = Resolve<ResourceRegistry<IFileInfo>>();
        RadarrCustomFormatResource[] resources =
        [
            new()
            {
                Id = 42,
                Name = "First CF",
                TrashId = "cf-one",
                TrashScores = new Dictionary<string, int> { ["default"] = 100 },
            },
            new()
            {
                Id = 42,
                Name = "Second CF",
                TrashId = "cf-two",
                TrashScores = new Dictionary<string, int> { ["default"] = 200 },
            },
        ];
        foreach (var resource in resources)
        {
            var json = JsonSerializer.Serialize(resource, GlobalJsonSerializerSettings.Guide);
            var path = $"/guide/radarr/cf/{resource.TrashId}.json";
            Fs.AddFile(path, new MockFileData(json));
            registry.Register<RadarrCustomFormatResource>([Fs.FileInfo.New(path)]);
        }
        ...
        var config = NewConfig.Radarr() with
        {
            ...
            [
                new CustomFormatConfig
                {
                    TrashIds = ["cf-one", "cf-two"],
                    AssignScoresTo = [new AssignScoresToConfig { Name = "WEB" }],
                },
            ],
            QualityProfiles = [new QualityProfileConfig { Name = "WEB" }],
        };
        var (sut, _) = CreatePlanBuilder(config);
