...
internal sealed class GitConfigTemplatesResourceProviderIntegrationTest : IntegrationTestFixture
{
    ...
    public async Task Supports_templates_for_both_services()
    {
        ...
        templates.Where(t => t.Service == SupportedServices.Radarr)
            .Should().HaveCount(2)
            .And.Subject.Select(t => t.Id).Should().BeEquivalentTo("radarr-uhd", "radarr-hd");

        templates.Where(t => t.Service == SupportedServices.Sonarr)
            .Should().HaveCount(2)
            .And.Subject.Select(t => t.Id).Should().BeEquivalentTo("sonarr-anime", "sonarr-web");
    }
    ...
    public async Task Templates_and_includes_can_coexist()
    {
        ...
        // Assert
        provider.GetTemplates().Should().HaveCount(1)
            .And.Subject.First().Id.Should().Be("test-template");
        provider.GetIncludes().Should().HaveCount(1)
            .And.Subject.First().Id.Should().Be("test-include");
    }
}
