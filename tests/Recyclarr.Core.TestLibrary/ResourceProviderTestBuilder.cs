...
public class ResourceProviderTestBuilder
{
    ...
    )
    {
        ...
        var config = new GitRepositorySource
        {
        ...
        return new GitTrashGuidesResourceProvider(config, repoUpdater, appPaths);
    }
    ...
    )
    {
        ...
        return new GitConfigTemplatesResourceProvider(config, repoUpdater, appPaths);
    }
}
...
public class TrashGuidesTestDataBuilder
{
    ...
    public TrashGuidesTestDataBuilder(MockFileSystem mockFileSystem, string providerName = "test-provider")
    {
        _mockFileSystem = mockFileSystem;
...
public class ConfigTemplatesTestDataBuilder
{
    ...
    public ConfigTemplatesTestDataBuilder(MockFileSystem mockFileSystem, string providerName = "test-provider")
    {
        _mockFileSystem = mockFileSystem;
    ...
    public ConfigTemplatesTestDataBuilder WithTemplates(params TemplateInfo[] templates)
    {
        ...
        {
            templates = [new TemplateInfo("test-template", SupportedServices.Radarr, "radarr/nested/deep/template.yml")];
        }

        var radarrTemplates = templates
            .Where(t => t.Service == SupportedServices.Radarr)
            .Select(t => new { Id = t.Id, Template = t.Path, Hidden = false })
            .ToArray();
    ...
    )
    {
        var radarrIncludes = includes
            ...
            .Select(i => new { Id = i.Id, Template = $"includes/{i.Id}.yml", Hidden = false })
            .ToArray();
